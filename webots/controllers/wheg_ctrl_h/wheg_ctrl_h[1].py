...
from wheg_utils.generators.mod_hopf_net import GeneratorHopf
...
class ControllerCPG:
    def __init__(self,n_whl,seed):
        ...
        self.cpg = GeneratorHopf(n_whl,cfg)
        self.n_whl = n_whl
    ...
    def control_math(self):
        ...
        # mixed up somewhere, patching here...
        # TODO fix??
        # tp = self.rot_tar[0]
        # self.rot_tar[0] = self.rot_tar[2]
        # self.rot_tar[2] = tp

        # tp = self.ext_tar[0]
        # self.ext_tar[0] = self.ext_tar[2]
        # self.ext_tar[2] = tp
    ...
    def read_commands(self):
        ...
        if self.time > 1000 and self.command_iter == 0:
            ...
            self.cpg.bias_inter[:] = np.zeros((4,4))
            self.command_iter = 1
            self.vector_callback([60,0.0,80,0,0])

        if self.time > 3000 and self.command_iter == 1:
            self.command_iter = 2
            self.vector_callback([60,0.0,120,0,0])
